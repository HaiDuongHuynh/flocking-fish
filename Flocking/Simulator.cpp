...
void Simulator::AddMember(int index, double x, double y, bool adv, bool control) {
  int i = index;
  ...
  Agent a_i;
  a_i.Init(i,pos,vel,mass,maxVel,maxAccel,viewRadius);
  a_i.SetAdversary(adv); // set adversary over here :D 
  a_i.SetControlled(control);
  agents.push_back( a_i );
}
...
void Simulator::Update() {
  //double dt = 0.1; //0.05;
	double dt = 0.025;
  for(int i=0; i<(int)agents.size(); i++) {
     //cout << "Fish No." << i << ": " << agents[i].GetHealth() << endl;
     if (agents[i].GetHealth() > 0) {
