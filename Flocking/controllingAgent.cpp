...
void createMenu() {
	...
	glutAddMenuEntry("Add Controlled", 4);
...
void mousebutton(int button, int state, int x, int y)
{
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
    ...
    if( addIndividual )
      gSim.AddMember(-1,tx,ty,false,false);
    else if( addGroup )
      for(int i=0; i<5; i++)
		gSim.AddMember(-1,tx,ty,false,false);
	...
	else if (addControlled) {
		gSim.AddMember(-1, tx, ty,false,true);
	}
  ...
  if( button == GLUT_MIDDLE_BUTTON && state == GLUT_DOWN ) {
	...
	double ty = -windowH / 2 + ((1.0 * windowH - y) / windowH) * (windowH);
	if (addIndividual)
		gSim.AddMember(-1, tx, ty,true,false); 
