...
void Agent::Update(vector<Agent>& agents, double dt) {
  ...
  if( isControlled ) {
	...
	if( dist < viewRadius && dist < closeDist ) {
	  ...
	  closeDist = dist;
	  numSeen++;
      agents[closestIndex].SetHealth(agents[closestIndex].GetHealth() - 1000);
	}
      }//endfor i
  ...
  for (int i = 1; i <= agents.size(); i++) {
      agents[i].SetHealth(agents[i].GetHealth() - 1);
  }
  */
}
...
void drawFish(double length) {
        glBegin(GL_POLYGON); // draw body

        glColor3f(0.0, 1.0, 0.0);
        glVertex2i(length - 50, length * 5 - 50);//x 5x 
        glVertex2i(length * 3 - 50, length * 7 - 50);//3x 7x
        glVertex2i(length * 8 - 50, length * 5 - 50);//8x 5x
        glVertex2i(length * 2.5 - 50, length * 4 - 50);//2.5x 4x
        glEnd();
        ...
        glColor3f(0.0, 1.0, 0.0);
        glVertex2i(length * 8 - 50, length * 5 - 50); //8x 5x
        glVertex2i(length * 9 - 50, length * 6 - 50); //9x 6x
        glVertex2i(length * 8.5 - 50, length * 5 - 50); //8.5x 5x
        glVertex2i(length * 9 - 50, length * 4 - 50); //9x 4x
        glVertex2i(length * 8 - 50, length * 5 - 50); //8x 5x
        glEnd();
        ...
        glVertex2i(length * 3 - 50, length * 7 - 50); //3x 7x
        glVertex2i(length * 3.5 - 50, length * 7.5 - 50); //3.5x 7.5x
        glVertex2i(length * 7 - 50, length * 5.4 - 50); //7x 5.4x
        glVertex2i(length * 3 - 50, length * 7 - 50); //3x 7x
        glEnd();
        ...
        glColor3f(1.0, 0.0, 0.0);
        glVertex2i(length * 2.5 - 50, length * 4 - 50); //2.5x 4x
        glVertex2i(length * 3.5 - 50, length * 5 - 50); //3.5x 5x
        glVertex2i(length * 3 - 50, length * 4.1 - 50); //3x 4.1x
        glVertex2i(length * 2.5 - 50, length * 4 - 50); //2.5x 4x
        glEnd();
}

void drawFish2(double length) {
    glBegin(GL_POLYGON); // draw body
...
void Agent::Draw() {
  ...
  */
  if( drawMode == 1 ) {
    ...
    glPushMatrix();
    glTranslatef(pos.GetX(), pos.GetY(), 0);
    glRotated( radToDeg(ori) + 180, 0,0,1);
    if (isAdversary) {
        drawFish2(radius);
    }
